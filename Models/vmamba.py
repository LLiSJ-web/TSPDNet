...
from Models.SS2D.csms6s import CrossScan, CrossMerge
...
class SS2Dv2:
    ...
    def forward_corev2(
        ...
        B, D, H, W = x.shape
        D, N = A_logs.shape   # D = K * d_inner   N = d_state
        ...
        if cascade2d:
            ...
            # xs = CrossScan.apply(x)   #　x: (B, D, H, W)   xs: (B, K, D, L)
            # P = prototype.shape[0]   # 6
            xs = CrossScan.apply(x)

            if prototype is None:
                raise ValueError("prototype is None.")
            
            if prototype.dim() != 2:
                raise ValueError(
                    f"prototype must be [P, D], got {prototype.shape}"
                )
            
            if prototype.shape[1] != D:
                raise ValueError(
                    f"prototype dim mismatch: "
                    f"prototype.shape={prototype.shape}, "
                    f"SS2D expected D={D}"
                )
            ...
            proto = prototype.transpose(0, 1)    # [D, 6]
            proto = proto.unsqueeze(0).unsqueeze(0)
            # [1, 1, D, 6]
            
            proto = proto.expand(B, K, -1, -1)
            # [B, K, D, 6]
            
            xs = torch.cat([proto, xs], dim=-1)
            # [B, K, D, 6 + L]
            ...
            if no_einsum:
                x_dbl = F.conv1d(xs.view(B, -1, L_scan), x_proj_weight.view(-1, D, 1),
                                 bias=(x_proj_bias.view(-1) if x_proj_bias is not None else None), groups=K)
                # reshape + split
                dts, Bs, Cs = torch.split(x_dbl.view(B, K, -1, L_scan), [R, N, N], dim=2)
                dts = F.conv1d(dts.contiguous().view(B, -1, L_scan), dt_projs_weight.view(K * D, -1, 1), groups=K)
            else:
            ...
            xs = xs.view(B, -1, L_scan)
            dts = dts.contiguous().view(B, -1, L_scan)
            As = -torch.exp(A_logs.to(torch.float))  # (k * c, d_state)
            Bs = Bs.contiguous().view(B, K, N, L_scan)
            Cs = Cs.contiguous().view(B, K, N, L_scan)
            ...
            ys = ys.view(B, K, -1, L_scan)
            ys_img = ys[..., P:] # [B, K, D, L]
            ys_img = ys_img.view(B, K, -1, H, W)
            y = CrossMerge.apply(ys_img)
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        x = self.in_proj(x)  # (B,h,w,d_model) → (B,h,w,d_proj)
        z = None
...
class VSSBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor, prototype=None):
        x = input
...
class VSSMEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prototypes=None):
        x_ret = []
        x_ret.append(x)
        ...
        if self.pos_embed is not None:
            pos_embed = (
                self.pos_embed.permute(0, 2, 3, 1).contiguous()
                if not self.channel_first
                else self.pos_embed
            )
            x = x + pos_embed
...
def load_pretrained_Base(model, ckpt_path=""):
    ...
    for k, v in ckpt['model'].items():
        ...
        if "classifier" in k:
            continue
        if "downsample" in k:
            match = re.findall(r"layers\.(\d+)\.downsample", k)

            if len(match) > 0:
                i_ds = int(match[0])

                k = k.replace(
                    f"layers.{i_ds}.downsample",
                    f"downsample.{i_ds}"
                )
        if ".blocks." in k:
            k = k.replace(".blocks.", ".")
        ...
        if k in model_dict:
            ...
            if v.shape == model_dict[k].shape:
                ...
                skipped_params.append(
                    f"{k}: checkpoint {v.shape}, model {model_dict[k].shape}"
                )
    ...
    print(
        f"[Pretrain] Loaded parameters: "
        f"{len(loaded_params)} / {len(model_dict)}"
    )

    if len(skipped_params) > 0:
        print(
            f"[Pretrain] Skipped parameters: "
            f"{len(skipped_params)}"
        )

        # 只显示前 20 个，避免刷屏
        for item in skipped_params[:20]:
            print("  ", item)
