...
test_datasets = ['TSOD']
gt_root = '/root/autodl-tmp/TSOD10K/Test/mask/'

def evaluate_model(model):
    results_list = []
    for dataset in test_datasets:
        salmap_root = os.path.join(dataset_path, model, dataset + '/')
        print(salmap_root)
        ...
        precision_save = os.path.join(dataset_path, model, 'precision.npy')
        recall_save = os.path.join(dataset_path, model, 'recall.npy')
        ...
        print('model: {} | dataset: {} || & {} & {} & {} & {} & {} & {} & {} & {}'.format(results['model'],
                                                                              ...
                                                                              str(results['adpFm_r']),
                                                                              str(results['maxFm_r']),
                                                                              str(results['meanFm_r']),
                                                                              str(results['adpEm_r']),
                                                                              str(results['maxEm_r']),
                                                                              str(results['meanEm_r']),
                                                                              str(results['Smeasure_r']),
                                                                              str(results['MAE_r'])))
        print(f'Wmeasure_r: {wfm.round(4)}')
    return results_list
